...
function isRetryableBootstrapError(err: unknown): boolean {
...
/**
 * Fetch /health/env with short retries on transient failures. This is a
 * bootstrap call the whole dashboard SSR depends on, so a bun-watch API
 * restart or a momentary 429/503 must not cascade into the "API unavailable"
 * screen.
 */
